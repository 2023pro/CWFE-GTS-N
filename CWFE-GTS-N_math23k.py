...
pairs, generate_nums, copy_nums, all_seq_words_nature, all_BO, ID = transfer_num(data)
...
pairs, _, _, all_test_seq_words_nature, all_BO_test, ID_test = transfer_num(data)
...
input_lang, output_lang, words_nature_lang, train_pairs, test_pairs = prepare_data_bert(pairs_trained, pairs_tested, 5, generate_nums,copy_nums,'weight/electra/vocab.txt',all_seq_words_nature,
                                                                                        all_test_seq_words_nature,all_BO, all_BO_test, ID,ID_test, tree=True)
...
encoder = Encoder_bert_rnn(input_dim=input_lang.n_words, w_n_dim=words_nature_lang.n_words, emb_dim=hidden_size,
                           hid_dim=hidden_size, n_layers=n_layer,
...
encoder_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(encoder_optimizer, mode='min', factor=0.5, patience=4,
                                                               verbose=True, threshold=0.1, threshold_mode='rel')
predict_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(predict_optimizer, mode='min', factor=0.5, patience=4,
                                                               verbose=True, threshold=0.1, threshold_mode='rel')
generate_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(generate_optimizer, mode='min', factor=0.5, patience=4,
                                                                verbose=True, threshold=0.1, threshold_mode='rel')
merge_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(merge_optimizer, mode='min', factor=0.5, patience=4,
                                                             verbose=True, threshold=0.1, threshold_mode='rel')
...
for epoch in range(n_epochs):
    ...
    num_size_batches, words_nature_input_batches, words_nature_input_length, BO_input_batches, BO_length = prepare_train_batch(
    ...
    print("--------------------------------")
    if epoch == 0 or epoch == 10 or epoch == 20 or epoch >= 30:
        value_ac = 0
        equation_ac = 0
        eval_total = 0
        start = time.time()
        for test_batch in test_pairs:
            test_res = evaluate_tree(test_batch[0], test_batch[1], generate_num_ids, encoder, predict, generate,
                                     merge, output_lang, test_batch[5], test_batch[7], test_batch[9], test_batch[10],
                                     beam_size=beam_size)
        ...
        if (float(value_ac) / eval_total) > best_val_acc:
            ...
            torch.save(encoder.state_dict(), "/home1/sq/CWFE-GTS-N_math23k/models/encoder")
            torch.save(predict.state_dict(), "/home1/sq/CWFE-GTS-N_math23k/models/predict")
            torch.save(generate.state_dict(), "/home1/sq/CWFE-GTS-N_math23k/models/generate")
            torch.save(merge.state_dict(), "/home1/sq/CWFE-GTS-N_math23k/models/merge")
            epoch_best = epoch
